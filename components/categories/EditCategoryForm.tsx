...
export default function EditCategoryForm({ categoryId, initialName }: EditCategoryFormProps) {
  ...
  async function onSubmit(data: CategoryFormData) {
    ...
    try {
      ...
      toast.success(res.data.message || "Category successfully updated");
    ...
    <div className="flex min-h-screen flex-col items-center justify-center py-12">
      <div className="w-full max-w-md rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#111] p-6 text-center shadow-sm transition-shadow duration-300 hover:shadow-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          {serverError && (
            <p className="text-red-500 dark:text-red-400 text-sm mt-2">{serverError}</p>
          )}
