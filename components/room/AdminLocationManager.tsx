...
export default function AdminLocationManager({ room, onRoomUpdate }: Props) {
  ...
  return (
    <div className="space-y-8 bg-gray-50/50 dark:bg-gray-900/10 rounded-2xl border border-gray-200/80 dark:border-gray-800/80 p-6 shadow-xs backdrop-blur-md">
      ...
      {/* Auto-generated locations list */}
      <div className="space-y-4">
        <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase tracking-widest flex items-center gap-2">
          <Sparkles className="w-3.5 h-3.5 text-violet-500" />
          <span>Auto-Generated ({autoGeneratedLocations.length})</span>
        </h3>
        ...
        {autoGeneratedLocations.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-3">
            {autoGeneratedLocations.map((loc) => (
              <div
                key={loc._id}
                className="p-4 rounded-xl bg-white dark:bg-[#111] border border-violet-200/60 dark:border-violet-800/40 flex flex-col sm:flex-row justify-between sm:items-center gap-3 transition-all"
              >
                <div className="min-w-0 flex items-start gap-2.5">
                  <div className="mt-0.5 shrink-0 w-5 h-5 rounded-md bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center">
                ...
                <span className="shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-300 text-xs font-semibold">
